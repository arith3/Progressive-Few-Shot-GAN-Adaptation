...
class Patch_Discriminator(nn.Module):
    ...
    def setConfig(self, ind = None, extra = None, flag = None, p_ind = None, real=False):
        self.ind = ind
        self.extra = extra
        self.flag = flag
        self.p_ind = p_ind
    ...
    def forward(self, inp):
        ...
        for i in range(len(self.convs)):
            if i == 0:
                ...
                temp1 = self.convs[i].conv1(inp)
                if (self.flag > 0) and (temp1.shape[1] == 512) and (temp1.shape[2] == 32 or temp1.shape[2] == 16):
                    feat.append(temp1)
                temp2 = self.convs[i].conv2(temp1)
                if (self.flag > 0) and (temp2.shape[1] == 512) and (temp2.shape[2] == 32 or temp2.shape[2] == 16):
                    feat.append(temp2)
                inp = self.convs[i](inp)
                if (self.flag > 0) and len(feat) == 4:
                    ...
                    inp = self.extra(feat[self.p_ind], self.p_ind)
                    return inp, feat
